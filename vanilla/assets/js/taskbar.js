const addHandlersTask = () => {
    ...
    prev_task.addEventListener('click', function() {
        ...
        if (task_windows.length > 0 && task_windows.length * 290 >= taskbar.offsetWidth) {

            for (let i = 0; i < task_windows.length; i++) gsap.to(task_windows[i], 0.3, { x: "+=290", ease:Power1.easeInOut });
